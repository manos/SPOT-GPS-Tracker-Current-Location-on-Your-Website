...
last_latlon_cache = "/home/open16/public_html/gps/lastspotlocation.txt"
json_cache = "/home/open16/public_html/gps/spotlocations.json"
xml_cache = "/home/open16/public_html/gps/spotlocations.xml"
map_output = "/home/open16/public_html/gps/mymap.html"
...
def do_map(json_output, options):
    ...
    for track in json_output:
        gmap.addpoint(track['latitude'], track['longitude'], "#0000FF",
                      title="%s%s" %(options.point_title_prefix, track['dateTime'])
                     )
...
if __name__ == '__main__':
    ...
    parser.add_option("-m", "--map", help="generates a google map with all points (and draws a line between them)",
                      action="store_true")
    parser.add_option("--map-zoom", help="zoom level for map", default=8)
    parser.add_option("--point-title-prefix",
                      help="for each point plotted, the prefix for the title (suffix is the dateTime provided by SPOT)",
                      default='Open16: ')
